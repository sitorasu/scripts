...
#   - 0: すべてのファイルのチェックが正常に完了し、フォーマットの違反がなかった
#   - 1: 全てのファイルのチェックが正常に完了し、フォーマットの違反があった
#   - 2: あるファイルのチェックが正常に完了しなかった
...
do
  ...
  # ファイルヘッダコメントの年号は更新されているか？
  if ! head -n 1 "$filename" | grep "$(date "+%Y")" > /dev/null; then
    echo "$filename:1: Year is not updated."
    ret=1
  fi
